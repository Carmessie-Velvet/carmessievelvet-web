...
- Cada columna cambia con un **slide horizontal** (`AnimatePresence` sin `mode="wait"` — para que la que sale y la que entra se muevan a la vez, no una tras otra — + `motion.div` con variants `enter`/`center`/`exit` en función de `custom={direction}`, Framer Motion) en vez de un corte instantáneo; a propósito no es un fade — feedback explícito de que se veía "de desaparecer y aparecer" y se quería el efecto de deslizar. `direction` se calcula comparando el índice clickeado contra `primaryIndex` (adelante = entra por la derecha, atrás = entra por la izquierda) y se le pasa a ambas columnas para que se muevan sincronizadas. El video no lo necesita porque nunca cambia. Las miniaturas van pegadas justo debajo del grid principal, sin margen propio (`gap-2` solo entre ellas, nada de `mt-*`/`pt-*` respecto a las imágenes grandes). Relación de aspecto `2:3` (`GALLERY_RATIO`, medida en vivo del sitio de referencia marsthelabel.com — sus miniaturas son 30×45px) tanto en las columnas grandes como en las miniaturas, más alta que el `4:5` original para que la prenda se aprecie completa.
