...
### Video de producto (`Product.videoUrl`, `VideoShowcase.tsx`, `ProductGallery.tsx`)

La API soporta un video por producto (`StoreProductDto.videoUrl: string | null`, gestionado por el admin — `POST`/`DELETE /products/:sku/video`, MP4/WEBM, máx. 50MB, sin placeholder: `null` hasta que se sube uno). El frontend solo lee este campo en dos lugares — no hay UI de subida aquí, eso vive en `carmessievelvet-admin`.

**Home (`VideoShowcase.tsx`)** — ya no es la sección estática de antes (4 clips locales fijos, sin relación a productos). Ahora es `"use client"`, recibe `products: Product[]` desde `page.tsx` (que pide `productService.getAll()` — hasta 100 productos, mismo límite que el resto del catálogo — y filtra por `videoUrl`) y renderiza uno por producto, ya sin título ni precio superpuesto (deliberado, a pedido del cliente: mantiene el look minimalista, el comprador descubre el producto al hacer clic). Si no hay ningún producto con video, la sección no se monta (`return null`).

- **Fila horizontal sin scrollbar visible, con "peek"**: los tiles usan un ancho fraccionario que a propósito no llena el contenedor (`w-[45%] sm:w-[30%] lg:w-[22%]`), así el siguiente tile siempre asoma en el borde como señal de que hay más — es la única pista de que se puede seguir, no hay flecha ni indicador aparte. El contenedor es `overflow-x-auto` con la barra de scroll ocultada por CSS (`[scrollbar-width:none]` + `[&::-webkit-scrollbar]:hidden`); en touch/trackpad el swipe nativo ya funciona solo, pero un mouse normal no puede arrastrar un `overflow-x-auto` por defecto, así que hay un handler de Pointer Events a mano (`pointerdown`/`pointermove`/`pointerup` sobre el contenedor, moviendo `scrollLeft` directamente) para que también se pueda arrastrar con clic sostenido.
- **Arrastrar vs. dar clic**: cada tile es un `<Link>` a `/producto/[slug]` — sin distinguir el gesto, arrastrar terminaría navegando por accidente al soltar sobre un tile. Se trackea la distancia total recorrida en el drag (`drag.current.moved`) y, si pasó de `DRAG_CLICK_THRESHOLD` (6px), el siguiente `click` se cancela con `preventDefault`/`stopPropagation` en fase de captura — un tap sin movimiento sigue navegando normal.
- Mismos atributos de siempre (`autoPlay muted loop playsInline`) — el video de producto que suba el admin debe seguir la misma convención de audio-stripped que ya se documentaba aquí (nunca subir un `.MOV`/`.MP4` original tal cual).
- `public/videos/` (los 3 clips estáticos viejos) se borró por completo — ya no hay ningún video local en el repo, todo sale de `videoUrl`.

**Ficha de producto (`ProductGallery.tsx`)** — reemplaza el grid simple de imágenes de antes. Layout de dos columnas (arriba) + fila de miniaturas (abajo), inspirado en marsthelabel.com:

- Caso trivial (una sola imagen y sin video): una sola imagen grande, sin miniaturas — no hay nada entre qué elegir.
- Con video: el video queda **fijo** en la columna izquierda (nunca cambia); la columna derecha muestra `images[primaryIndex]`. Las miniaturas de abajo son solo las imágenes (el video no aparece ahí — ya está fijo arriba, agregarlo como miniatura sería redundante y confuso si no hace nada al hacer clic). Clic en una miniatura → solo cambia la columna derecha.
- Sin video (2+ imágenes): clic en una miniatura pone esa imagen a la izquierda y la siguiente (`(i + 1) % images.length`, con wraparound) a la derecha — por defecto muestra `images[0]`/`images[1]`. El wraparound es a propósito: al hacer clic en la última imagen, no se quiere que la columna derecha se quede vacía.
- Estado (`primaryIndex`) vive en el propio componente (`"use client"`). Clic en cualquier imagen grande (no en el video — ya se reproduce grande, no necesita "verse más grande") abre `ImageLightbox.tsx`: visor de pantalla completa (fondo `bg-ink`) con contador `N / total`, flechas prev/siguiente (con wraparound, también con flechas del teclado), fila de miniaturas propia, y cerrar con la X, `Escape`, o clic en el fondo. Recorre siempre el arreglo completo `images` (no el par que esté mostrando la galería en ese momento) — una vez adentro, es solo "ver todas las fotos", no importa cómo estén emparejadas arriba. `lightboxIndex` (estado separado de `primaryIndex`) guarda cuál imagen se abrió; cada imagen grande sabe su propio índice real (`leftIndex`/`rightIndex` en el caso sin video) para abrir el lightbox en la que se clickeó, no siempre en la 0.
- El contenido cambia con un **slide horizontal** (`AnimatePresence` sin `mode="wait"` + `motion.div` con variants `enter`/`center`/`exit` en función de `custom={direction}`, Framer Motion) en vez de un corte instantáneo o un fade — feedback explícito de que se veía "de desaparecer y aparecer" y se quería el efecto de deslizar. `direction` se calcula comparando el índice clickeado contra `primaryIndex` (adelante = entra por la derecha, atrás = entra por la izquierda).
  - **Con video**: solo la columna derecha cambia, así que es una sola caja deslizante (`SlidingBox`) — no hay nada que mantener sincronizado.
  - **Sin video**: las dos columnas (el par completo) son **un solo componente deslizante** — un único `SlidingBox` con `key` = ambos `src` combinados, y el `grid grid-cols-2` de las dos imágenes vive *adentro* de esa caja. Primer intento fue animar cada columna por separado (con un `delay` para diferenciarlas) — feedback explícito de que igual se sentía como una sola imagen ancha, y de que el modelo correcto es al revés: las dos imágenes SON un solo componente, no dos independientes. El contenedor exterior usa `aspect-[4/3]` (dos columnas de `2:3` una junto a otra duplica el ancho, no el alto).
- Relación de aspecto `2:3` (`IMAGE_RATIO`, medida en vivo del sitio de referencia marsthelabel.com) en columnas grandes y miniaturas — más alta que el `4:5` original para que la prenda se aprecie completa. Miniaturas de `80×120px` (`w-20` + `aspect-[2/3]`), más grandes que el primer intento (`64×96px`) — también ajustado por feedback directo comparando con la referencia. El gap entre las columnas grandes es `gap-px` (casi cero, como la referencia); entre miniaturas es `gap-2`; entre el grid principal y la fila de miniaturas es `mt-2` — un espacio chico pero real, no cero (primer intento las pegaba directamente, feedback explícito de que la referencia sí tiene un espacio pequeño ahí).

### Hero de la home (`GET /store/hero`)

El banner de la home (imagen a pantalla completa + eyebrow/título + botón) ya no está hardcodeado — sale de `GET /api/v1/store/hero` (público, ver sección 21 de `../carmessievelvet-api/docs/API-FRONTEND.md`). El contenido y la imagen se administran **desde `carmessievelvet-admin`, no desde este repo** — aquí solo se lee y se pinta.

- `src/types/hero.ts` + `src/services/hero-service.ts` (`heroService.getActive()`) — la API es un catálogo (varios heroes activos = carrusel, por `sortOrder`), pero hoy solo se pinta `heroes[0]`; no hay UI de carrusel todavía porque nadie lo ha pedido — si el admin llega a activar más de uno, sólo se ve el primero, no es un bug.
- `title`/`content`/`buttonLabel`/`buttonPath` vienen `null` si el admin apagó ese switch (`showTitle`/`showContent`/`showButton`) — cada uno se pinta con su propio `{campo && (...)}`, nunca asumir que los cuatro vienen juntos. Es justo cómo se resuelve "el botón es opcional": si `buttonLabel`+`buttonPath` no vienen, no se renderiza nada, ni un botón vacío.
- `GET /store/hero` devuelve `[]` mientras el admin no active ningún hero — para que la home nunca se quede sin banner mientras tanto, `page.tsx` tiene un `FALLBACK_HERO` con el contenido original del sitio ("Nueva colección" / "Vestir con la textura de lo memorable." / "Ver colección" → `/tienda`, la misma imagen local de siempre). En cuanto haya un hero activo en la API, el fallback deja de usarse solo.
- `buttonPath` ya viene validado por el backend como ruta relativa (`/...`, nunca absoluta) — se usa tal cual como `href` de un `Link`, no hace falta sanitizarlo de este lado.
