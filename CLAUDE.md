...
- Cada columna cambia con un **slide horizontal** (`AnimatePresence` sin `mode="wait"` — para que la que sale y la que entra se muevan a la vez, no una tras otra — + `motion.div` con variants `enter`/`center`/`exit` en función de `custom={direction}`, Framer Motion) en vez de un corte instantáneo; a propósito no es un fade — feedback explícito de que se veía "de desaparecer y aparecer" y se quería el efecto de deslizar. `direction` se calcula comparando el índice clickeado contra `primaryIndex` (adelante = entra por la derecha, atrás = entra por la izquierda) y se le pasa a ambas columnas. Cuando las dos cambian a la vez (caso sin video), la columna derecha lleva un `delay` corto (`GallerySlot`, prop `delay`, 0.08s) respecto a la izquierda — sin ese desfase, moverse exactamente sincronizadas leía como "una sola imagen ancha deslizándose" en vez de dos fotos independientes (feedback explícito también). El video no lleva delay porque nunca cambia. Relación de aspecto `2:3` (`GALLERY_RATIO`, medida en vivo del sitio de referencia marsthelabel.com — sus miniaturas son 30×45px) tanto en las columnas grandes como en las miniaturas, más alta que el `4:5` original para que la prenda se aprecie completa. El gap entre las dos columnas grandes es `gap-px` (casi cero, como en la referencia) — las miniaturas sí llevan `gap-2` entre ellas, van pegadas (sin `mt-*`/`pt-*`) justo debajo del grid principal.
