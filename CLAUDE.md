...
- Cada columna cruza con fade (`AnimatePresence mode="wait"` + `motion.div` keyed por `image.src`, Framer Motion — mismo patrón que el resto del sitio) en vez de un corte instantáneo al cambiar de imagen; el video no lo necesita porque nunca cambia. Las miniaturas van pegadas justo debajo del grid principal, sin margen propio (`gap-2` solo entre ellas, nada de `mt-*`/`pt-*` respecto a las imágenes grandes).
