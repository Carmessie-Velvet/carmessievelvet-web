...
// A SIMPLE line's identity is product+size+color (a size can now come in
// more than one color, see `Product.colors`); a SET line has no single size,
...
export function cartItemKey(
  item: Pick<CartItem, "product" | "size" | "color" | "selections">
): string {
  if (item.selections) {
